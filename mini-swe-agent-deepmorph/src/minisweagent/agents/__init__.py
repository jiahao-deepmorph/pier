...
def get_agent(
    model: Model, env: Environment, config: dict, *, default_type: str = "", tools: dict | None = None
) -> Agent:
    config = copy.deepcopy(config)
    agent_class = get_agent_class(config.pop("agent_class", default_type))
    config_tools = config.pop("tools", None)
    return agent_class(model, env, tools=tools if tools is not None else config_tools, **config)
