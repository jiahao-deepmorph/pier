...
class InteractiveAgent(DefaultAgent):
    ...
    def execute_actions(self, message: dict) -> list[dict]:
        ...
        commands = [self._format_action_for_confirmation(action) for action in actions]
        outputs = []
        try:
    ...
    def _format_action_for_confirmation(action: dict) -> str:
        ...
        return f"{action.get('tool')}({json.dumps({k: v for k, v in action.items() if k != 'tool_call_id'}, sort_keys=True)})"
