...
def test_host_side_tool_dispatch(default_config):
    class RecordingTool:
        def __init__(self):
            self.action = None
        ...
        def execute(self, action):
            self.action = action
            return {"output": "graph result", "returncode": 0, "exception_info": ""}

        def serialize(self):
            return {"info": {"config": {"tools": {"graph_query": {"test": True}}}}}

    tool = RecordingTool()
    agent = DefaultAgent(
        model=make_tc_model([]),
        env=LocalEnvironment(),
        tools={"graph_query": tool},
        **default_config,
    )

    result = agent.execute_action(
        {"tool": "graph_query", "method": "getCallChain", "arguments": {"function_name": "createAccount"}}
    )

    assert result["output"] == "graph result"
    assert tool.action["method"] == "getCallChain"


def test_unknown_host_side_tool_returns_observation(default_config):
    agent = DefaultAgent(model=make_tc_model([]), env=LocalEnvironment(), **default_config)

    result = agent.execute_action({"tool": "missing_tool"})

    assert result["returncode"] == -1
    assert "Unknown runtime tool" in result["exception_info"]
