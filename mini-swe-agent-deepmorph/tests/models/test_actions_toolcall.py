...
class TestParseToolcallActions:
    ...
    def test_valid_graph_query_tool_call(self):
        tool_call = MagicMock()
        tool_call.function.name = "graph_query"
        tool_call.function.arguments = '{"method": "getCallChain", "arguments": {"function_name": "createAccount"}}'
        tool_call.id = "call_graph"
        assert parse_toolcall_actions([tool_call], format_error_template="{{ error }}") == [
            {
                "tool": "graph_query",
                "method": "getCallChain",
    ...
    def test_missing_graph_query_method_raises_format_error(self):
        tool_call = MagicMock()
        tool_call.function.name = "graph_query"
        ...
        with pytest.raises(FormatError) as exc_info:
            parse_toolcall_actions([tool_call], format_error_template="{{ error }}")
        assert "Missing 'method' argument" in exc_info.value.messages[0]["content"]
