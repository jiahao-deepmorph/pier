...
def test_graph_query_tools_are_wired_to_model_and_agent():
    with (
        patch("minisweagent.run.mini.configure_if_first_time"),
        patch("minisweagent.run.mini.get_agent") as mock_get_agent,
        patch("minisweagent.run.mini.get_model") as mock_get_model,
        patch("minisweagent.run.mini.get_environment") as mock_get_env,
        patch("minisweagent.run.mini.get_config_from_spec") as mock_get_config,
        patch("minisweagent.run.mini.get_tools") as mock_get_tools,
        patch("minisweagent.run.mini.get_tool_schemas") as mock_get_tool_schemas,
        patch("minisweagent.run.mini.get_response_api_tool_schemas") as mock_get_response_api_tool_schemas,
    ):
        runtime_tools = {"graph_query": Mock()}
        tool_schema = {"type": "function", "function": {"name": "graph_query"}}
        response_api_tool_schema = {"type": "function", "name": "graph_query"}
        mock_get_tools.return_value = runtime_tools
        mock_get_tool_schemas.return_value = [tool_schema]
        mock_get_response_api_tool_schemas.return_value = [response_api_tool_schema]
        mock_get_config.return_value = {
            "agent": {"system_template": "test"},
            "model": {},
            "environment": {},
            "tools": {"graph_query": {"enabled": True, "endpoint": "http://127.0.0.7:8765/mcp"}},
        }
        mock_get_model.return_value = Mock()
        mock_get_env.return_value = Mock()
        mock_agent = Mock()
        ...
        mock_get_tools.assert_called_once_with(
            {"graph_query": {"enabled": True, "endpoint": "http://127.0.0.7:8765/mcp"}}
        )
        model_config = mock_get_model.call_args.kwargs["config"]
        assert model_config["tool_schemas"] == [tool_schema]
        assert model_config["response_api_tool_schemas"] == [response_api_tool_schema]
        assert mock_get_agent.call_args.kwargs["tools"] == runtime_tools
        mock_agent.run.assert_called_once_with("Test graph tool wiring")
