...
from pier.cli.notifications import show_registry_hint_if_first_run
...
def _pier_hub_visibility(public: bool | None):
...
async def _setup_pier_hub_streaming(job, *, public: bool | None):
    ...
    Failure handling distinguishes two cases:
      ...
      * **Anything else** (transient network error, RPC blip, etc.) → log a
        ...
        post-run :func:`_run_pier_hub_upload` will do the full batch
    ...
    visibility = _pier_hub_visibility(public)
    uploader = Uploader()
    try:
        ...
        console.print(
            f"[yellow]Warning:[/yellow] Could not register job with Pier Hub "
...
async def _run_pier_hub_upload(job_dir: Path, *, public: bool | None) -> None:
    """Post-run finalize for `pier run --upload`.
    ...
    try:
        ...
        console.print(
            f"Uploaded to Pier Hub: "
            f"{PIER_VIEWER_JOBS_URL}/{result.job_id} "
        ...
        console.print(
            f"Retry with `pier upload {job_dir}{retry_flag}`", soft_wrap=True
        )
...
def start(
    ...
    registry_url: Annotated[
        ...
        Option(
            ...
            rich_help_panel="Dataset",
            show_default="The default pier registry.",
        ),
    ] = None,
    ...
    upload: Annotated[
        ...
        Option(
            "--upload",
            help="After the job finishes, upload it to Pier Hub so you can "
            "share the run via a link.",
            rich_help_panel="Pier Hub",
        ),
    ] = False,
    public: Annotated[
        bool | None,
    ...
    # Pier Hub flag validation: --public/--private requires --upload so the
    ...
    # Auth check for --upload happens inside `_setup_pier_hub_streaming`,
    ...
    if path is not None:
        ...
        if task_ref is not None:
            from pier.models.package.reference import PackageReference
    ...
    async def _run_job():
        ...
        # Streaming Pier Hub upload: insert the job row + register the
        ...
        if upload:
            await _setup_pier_hub_streaming(job, public=public)
        ...
        # Finalize must run on the same event loop as `_setup_pier_hub_streaming`
        ...
        if upload:
            await _run_pier_hub_upload(job.job_dir, public=public)
        else:
            console.print(f"Share results by running `pier upload {job.job_dir}`")
        console.print()
    ...
    # the long comment in `_run_pier_hub_upload` for why this matters.
    job, job_result = run_async(_run_job())

    if export_traces:
        from pier.utils.traces_utils import export_traces as _export_traces
...
def resume(
    ...
    upload: Annotated[
        ...
        Option(
            "--upload",
            help="After the resume finishes, upload the job to Pier Hub. "
            "If the job was already partially uploaded (e.g. from a previous "
            "`pier run --upload` that crashed), this fills in the missing "
    ...
    async def _run_job():
        ...
        # long comment in `_run_pier_hub_upload` for why.
        if upload:
            await _run_pier_hub_upload(job.job_dir, public=public)
        return job_result
...
def summarize(
    ...
    console.print(
        "[red]Error: 'pier jobs summarize' has been removed. "
        "Use 'pier analyze <job-dir>' instead.[/red]"
    )
    raise SystemExit(1)
...
def download(
    ...
    """Download a job (and all its trials) from the Pier platform."""
    ...
    async def _download() -> None:
        from pier.download.downloader import Downloader
        ...
        # summary + suggested `pier view` / `pier analyze` commands
        ...
        echo(f"Inspect locally by running `pier view {result.output_dir.parent}`")
        echo(f"Analyze trajectories by running `pier analyze {result.output_dir}`")
