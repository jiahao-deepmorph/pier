...
from pier.constants import NOTIFICATIONS_PATH
...
def show_registry_hint_if_first_run(console: Console) -> None:
    ...
    console.print(
        "[dim]Tip: There are many benchmarks available in Pier's registry.[/dim]"
    )
    console.print(
        "[dim]Run `pier datasets list` to see all available datasets.[/dim]"
    )
    console.print()
